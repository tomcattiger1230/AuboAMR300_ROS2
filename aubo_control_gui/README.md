...
关键位置另见 [四个存储位、放置位及过渡点验证](KEY_POSITION_VALIDATION.md)：2026-09-16 修正后的 finger 模型四个原存储位已通过碰撞、规划与 Isaac 执行；旧长夹爪碰撞结果保留为历史记录，默认快捷位配置保持原样。
...
原四个存储位的 J3 均在该范围内，单独校正 J3 不会消除旧长夹爪的碰撞；2026-09-16 使用修正后的 finger 末端复测四个位置已通过，见关键位置验证报告。
...
修改目标、规划参数、目标页签、碰撞场景，或起始关节状态偏离/反馈过期，都会使旧轨迹失效。MoveIt 规划场景包含外部碰撞几何时，机体场景坐标变换累计超过 1 mm / 0.001 rad 也会使轨迹失效；空场景中的 `odom → base_footprint` 仿真漂移不影响以车体为基准的机械臂轨迹，不再清除预览。关节起点检查为机械臂 0.01 rad、夹指 0.003 m；反馈时效 1.5 s。
