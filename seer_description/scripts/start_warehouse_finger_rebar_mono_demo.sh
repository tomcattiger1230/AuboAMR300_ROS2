...
REBAR_USD=""
for candidate in \
  "$SCRIPT_DIR/../urdf/warehouse_finger_rebar_mono_demo.usda" \
  "$SCRIPT_DIR/../../share/seer_description/urdf/warehouse_finger_rebar_mono_demo.usda"; do
  if [[ -f "$candidate" ]]; then
...
if [[ -z "$REBAR_USD" ]]; then
  printf 'warehouse_finger_rebar_mono_demo.usda was not found next to the installed package.\n' >&2
  exit 1
fi
