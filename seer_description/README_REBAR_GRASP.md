...
新增 `rebar_onboard_rack.usda` 为四个料槽，每槽在 Y=±0.19 m 设两个弧形托座，中央留给夹爪。托座外侧保留内半径 17 mm、开口 140° 的弧形挡边，底部使用 ±30° 的 V 形承托面；钢筋最终中心高度为 0.715 m。托座立柱底面为 Z=0.602 m，高度从 50 mm 增加到 95 mm，托座整体抬高 45 mm；下方新增两条实体安装底梁，衔接车体实际网格顶面。料槽是 `base_link` 刚体下的碰撞子几何，跟随车体运动，不是悬空的世界固定物体。
...
抓取和提起后，MoveIt 将 0.6 m 钢筋作为夹爪上的 attached collision object；继续升高，绕竖直方向旋转 90°，保持腕部向下、钢筋水平，以三段笛卡尔路径从车体侧面绕行转移，在料槽上方缓慢下探至原标定释放目标 Z=0.720 m，钢筋中心比落座中心高 5 mm，松开让钢筋落入托座，再撤回夹爪。所有路径启用碰撞检查。搬运规划使用半径 20 mm、长度 0.62 m 的保守包络；最后下探时半径缩为 14 mm（实际半径 12 mm，仍留 2 mm 余量），避免搬运包络在 5 mm 释放间隙处提前碰到托座。机械臂、钢筋、相邻槽与托座的规划碰撞检查继续启用。每 0.25 s 比较实际钢筋中心与夹爪 TCP；偏差超过 40 mm 持续 0.35 s 即取消机械臂执行，丢失坐标反馈超过 1 s 也会停止。各段结束额外检查钢筋实际跟随误差小于 25 mm。`--clearance-height` 默认 0.90 m；工位远端升到 0.98 m 会接近当前姿态的可达边界，已调低避开。成功判断使用钢筋实际 TF：落点、Y 轴方向、夹指张开和释放后的稳定性，均相对动态车体反馈计算。
...
预装钢筋仍是独立动态刚体，使用相同尺寸、密度和材料，依靠托座承载；MoveIt 也加载它们的碰撞几何。测试除了新钢筋落位，还核查其余三根仍在各自槽内。释放目标与料槽高度分别在 `rebar_experiment_geometry.py` 的 `RACK_RELEASE_TCP_Z` 和 `RACK_REBAR_Z` 中定义；预装钢筋、USD 托座和 MoveIt 碰撞模型使用同一组参数。
...
## 2026-09-16 初版车载实测结果（Z=0.670 m，历史记录）

四个料槽分别从复位场景完成实际抓取、90° 转向、侧面搬运、下探、释放和撤回，各 24 项检查全部通过（空槽独立测试）。当时使用 0.90 m 搬运高度、座面上方 50 mm 释放高度；短时静置检查通过，但后续长时观察仍发现轻微滚动，并有一次释放弹出，不能据此认定料槽长期稳定。下述数值对应旧托座高度，不代表抬高后的版本。
...
## 托座抬高与防滚修正（2026-09-16）

用户观察到钢筋在槽内轻微滚动，另一次释放时弹出。[30 s 墙钟观测](test/results/rebar_jitter_baseline_20260916.json)（约 11.2 s 仿真时间）确认旧槽内四根钢筋相对底盘的 X 跨度约 0.36–0.55 mm、角度变化约 1.14–2.14°；世界坐标还混有底盘自身缓慢漂移，需分别记录。单纯减小接触偏移并增加阻尼的一轮试验仍滚动且[释放失败](test/results/rebar_contact_grasp_20260916.json)，该组合已撤回。

当前四槽的八个托座增加 V 形承托面，弧底中间八片仅显示，避免多片近水平接触竞争；外侧挡边仍参与碰撞。各钢筋设置接触求解位置迭代 32、速度迭代 8，保留原摩擦、接触偏移、阻尼、尺寸、密度和自由刚体运动。按用户要求加高托座基座 45 mm，释放目标保持 0.720 m，将自由落差从 50 mm 降至 5 mm；机械臂安装基座和学生原始位置不变。

重启场景并完成装载撤回后，可用只读 TF 观测区分槽内滚动与底盘漂移：

```bash
ros2 run seer_description measure_rebar_settling.py --seconds 30 \
  --output /tmp/rebar_settling.json
```

默认同时测量四根钢筋；空槽场景使用 `--bars rebar`。报告同时记录墙钟/仿真时长、底盘与世界坐标的位移跨度和角度变化；默认底盘相对跨度上限 0.1 mm、角度上限 0.25°，超限或缺失 TF 返回非零。

本轮抬高后[装载实测](test/results/rebar_raised_vseat_grasp_20260916.json)为槽 2/3/4 预装、槽 1 实际抓取装载，26 项全部通过。新钢筋释放前中心 Z=0.719331 m、落座后 Z=0.715315 m，实际下降约 4.02 mm，夹爪正常张开并撤回，无弹出，其余三根仍在槽内。USD 一致性校验 9 项通过，网页黑白视频返回新鲜图像（HTTP 200）。抬高版本尚未重新逐槽独立执行槽 2/3/4，旧四槽报告继续保留为历史记录。

[紧接撤回的首段观测](test/results/rebar_raised_settling_20260916.json)中，新放入钢筋相对底盘 X 跨度 0.805 mm、角度变化 4.00°，未通过严格静置阈值；[随后一段](test/results/rebar_raised_settling_late_20260916.json)降为 0.267 mm、1.11°，仍未通过。三根预装钢筋各段最大相对位移跨度小于 0.008 mm、角度小于 0.003°。这表明新钢筋还有落座收敛过程，不能将流程通过表述为释放后立即完全静止。

继续自然落座后的[稳态观测](test/results/rebar_raised_settled_20260916.json)（30 s 墙钟、10.3 s 仿真）四根全部通过：新钢筋相对底盘最大位移跨度 0.00351 mm、角度 0.00827°，其余三根分别小于 0.0025 mm、0.0011°。未冻结钢筋或增加固定连接。当前结论是降低落差后本轮未弹出、短暂落座滚动最终收敛；不是释放瞬间完全静止，也不是每个新高度料槽均已重测。

同一稳态段世界坐标 X 跨度约 2.68–2.77 mm、姿态变化约 0.448°，但底盘相对钢筋几乎不动，表明仍有底盘整体缓慢漂移。带四根钢筋场景与之前空载底盘回归的负载和接触状态不同，旧空载数值不能替代本段测量。本次未进一步修改轮驱动或底盘控制。静置验证应等落座收敛后进行；当前带视频场景的 30 s 墙钟仅约 10 s 仿真，需结合报告中的两种时长判断。

## 接触车体的实体安装底梁（2026-09-16）

此前按车体简化碰撞盒顶面布置的托座立柱底面为 Z=0.602 m，但 Isaac 组合模型中 `robot_body` 的实际视觉顶面为 Z≈0.598651 m，留有约 3.35 mm 缝隙，看上去悬空。新增两条贯穿四槽的实体底梁 `MountingBeam1/2`，分别位于 Y=±0.19 m；每条覆盖该侧四根立柱的完整底面，中央仍留给夹爪。

| 项目 | 值（`base_link` 坐标） |
|---|---|
| 底梁中心 X | 0.15225 m |
| 底梁中心 Y | −0.19 / +0.19 m |
| 长 × 宽 × 厚 | 0.3649 × 0.100 × 0.020 m |
| 底面 / 顶面 Z | 0.59765 / 0.61765 m |
| 与车体网格接合 | 下沉约 1 mm，消除显示缝隙 |
| 与托座立柱接合 | 顶面高于立柱底面，实体交叠连接 |

安装底梁与托座均属于 `base_link` 下的碰撞子几何，随车体一起运动。Isaac USD 与 MoveIt 共用 `rack_boxes()`，料架合计 138 个几何，其中 74 个启用 Isaac 碰撞；底梁采用较深的灰蓝色便于识别。钢筋落座中心仍为 0.715 m、释放目标仍为 0.720 m。

修改安装面或底梁尺寸时，编辑 `scripts/rebar_experiment_geometry.py` 的 `RACK_DECK_Z`、`RACK_BASE_EMBED`、`RACK_BASE_THICKNESS`，运行 `python3 scripts/generate_rebar_station.py` 后重启场景。新增几何校验检查实际 USD 网格顶面、底梁接触车体、八根立柱均得到覆盖，以及规划与 USD 的位置一致性。

实体底梁版本的[完整装载复测](test/results/rebar_mounting_grasp_20260916.json)（槽 2/3/4 预装，槽 1 实际装入）26 项全部通过，包括下探、释放、夹爪撤回和其余三根仍在槽内；新钢筋落座中心 Z=0.715237 m，撤回后三秒位移约 0.283 mm。USD 几何校验 10 项通过。
